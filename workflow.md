...
目前總覽頁採三欄式配置：左側顯示觀測狀態和即時摘要，中間顯示海嘯提醒及測站地圖，右側提供底圖與氣象圖層控制。標題使用 Streamlit 原生標題元件，避免自訂 HTML 標題被版面裁切。更新全部資料的入口只保留在左側導覽列；地圖內不再放置重複的底圖/圖層控制，統一由右側面板操作。未啟用的測站圖層不建立，地圖互動結果不回傳 Streamlit，以減少地圖資料量與不必要的整頁重繪。

總覽另提供可展開的「六項資料新鮮度與更新狀態」表，列出各資料集最近擷取狀態、擷取時間及來源資料時間。固定週期資料依更新間隔設定逾時門檻（容許兩倍更新週期）；海嘯與颱風資料為事件型，舊事件時間不直接判為逾期。此狀態反映最近一次擷取紀錄，重新擷取仍由左側「更新全部資料」觸發；失敗或來源無可呈現資料須與資料過期分開顯示。
...
- 測站頁保留單站趨勢，並提供多測站趨勢比較：可選擇最多五站與一項觀測指標，比較各站最近 48 筆資料；不同測站的觀測時間不一致時保留缺值，不插補或偽造資料。
